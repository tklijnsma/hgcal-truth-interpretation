    ...
    if get.sum()[0] == 0:
        ...
        s = (
            ...
            .format(
                ...
                energy_atbound = event[b'simtrack_momentumAtBoundary'][get][0,0].E,
...
class Plot3DSingleEndcap(PlotBase):
    ...
    def plot_both_endcaps(
        ...
        if cls.SAVEPLOTS:
            ...
            if show: show_inline_matplotlib_plots()
        return fig
    ...
    def plot_tracks(self, trim=True):
        ...
        track_energy_at_boundary = self.event[b'simtrack_momentumAtBoundary'][0].E
        ...
        get_max = lambda a: np.max(np.abs(a))

        logger.info(
            'x_in_max = %s, y_in_max = %s, x_out_max = %s, y_out_max = %s',
            get_max(x[:,0]), get_max(y[:,0]), get_max(x[:,-1]), get_max(y[:,-1])
            )
...
def plot_decay_tree(
    ...
    for i_event, event in enumerate(data.iterate_events(progressbar=progressbar)):
        if not(istart is None) and i_event < istart: continue
        logger.info('Event %s', i_event)
        if not save is None: DecayTree.SAVEPLOTS = save
        if title: DecayTree.title = title.replace('%i', str(i_event))
        inst = DecayTree(event)
        inst.with_labels = with_labels
        inst.plot_graph()
        n_plotted += 1
        if not(nmax is None) and n_plotted >= nmax: break
    return event
...
class DecayTree(PlotBase):
    ...
    def __init__(self, event):
        ...
        self.n_tracks = self.event[b'simtrack_z'][0].shape[0]
        self.track_id = event[b'simtrack_trackId'][0]
        self.track_pdgid = event[b'simtrack_pdgid'][0]
        self.track_noparent = event[b'simtrack_noParent'][0]
        self.track_energy = event[b'simtrack_momentum'][0].E

        self.track_vertex_index = event[b'simtrack_vertexIndex'][0]
        self.vertex_parent_track_index = event[b'simvertex_parentTrackId'][0]
    ...
    def make_graph(self):
        ...
        nodes_array = np.array(
            [{'pdgid': self.track_pdgid[i], 'energy' : self.track_energy[i]} for i in range(self.n_tracks)],
            dtype=object
            )
        nodes = np.stack((self.track_id, nodes_array)).T
        G.add_nodes_from(nodes)

        # Edges are child-parent relations; only valid if track has a parent of course
        edge_o = self.track_id[~self.track_noparent]
        # edge_i should be the parent track indices, have to get them via the vertex
        edge_i = self.vertex_parent_track_index[self.track_vertex_index[~self.track_noparent]]
